...
all = []
for i in range(50):
    data = pd.read_csv(r'./landUse_normalize.csv', index_col=False)
    ...
    verifyData.to_csv(f'./verify_{i}.csv', index=False, header=False)
    # try:
        # # NBN
    ...
    pd.DataFrame(y_pred_lr).to_csv(f'./LR_y_pred_{i}.csv', index=False, header=False)
    ...
    log['kappa'].append(accuracy[2])
    log['rf_time'] = timeLog
    pd.DataFrame(y_pred_rf).to_csv(f'./RF_y_pred_{i}.csv', index=False, header=False)
    ...
    pd.DataFrame(y_pred_ann).to_csv(f'./ANN_y_pred_{i}.csv', index=False, header=False)
    ...
    #     all.append(log)
    #     print(log)
    #     with open(f'./log.txt', mode='a+') as edge:
    #         edge.write(str(log))
